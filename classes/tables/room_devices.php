<?php
	class Room_devices extends Tables{
		...
		private function sort_up_by_device_name($data){
			if(!empty($data["data"])){
				usort($data["data"], $this->build_sorter("room_devices_name"));
		...
		public function get($where_values){
			return $this->encode_output($this->sort_up_by_device_name($this->mysql->select_query($this->rd, $where_values)));
		}
